...
class RtmpFD(FileDownloader):
	    def real_download(self, filename, info_dict):
	    	   def run_rtmpdump(args):
	    	   	   start = time.time( )
	    	   	   resume_percent = None
	    	   	   resume_download_data_len = None
	    	   	   proc = Popen(args, stderr=subprocess.PIPE)
	    	   	   cursor_in_new_line = True
	    	   	   proc_stderr_closed = False
	    	   	   try:
	    	   	   	 while not proc_stderr_close:
	    	   	   	 	    ...
	    	   	   	 	    if mobj:
	    	   	   	 	    	   dowloaded_data_len = int(float(mobj.group(1)) * 1024)
	    	    	...
	    	    	# Download using rtmpdump. rtmpdump return exit code 2 when 
	    	    	# the connection was interrupted and resuming appears to be 
	    	    	#possible. This is part of rtmpdump's normal usage, AFAIK.
	    	    	basic_args = [ 
	    	    	        'rtmpdump', '--verbuse', '-r', url, 
	    	    	        '-o', tmpfilename]
	    	    	...
	    	    	try: 
	    	           ...
	    	           if retval == RD_SUCCESS or (test and retval == RD_INCOMPLETE):
	    	           	...
	    	           	self.to_screen('[rtmpdump] Download %s bytes' % fsize)
